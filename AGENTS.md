...
(`firmware/`, `host/`), plus an unrelated 3D-printed optics project
(`optics/`) that happens to live in the same repo — a prime-focus
microscope camera, superseding the earlier LEGO/eyepiece-collar plan
that older text in `optics/README.md` still describes. Part of the
